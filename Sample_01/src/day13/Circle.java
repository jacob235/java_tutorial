...
abstract class Shape{		// abstract는 틀을 잡아준다.(기반을 잡아준다)
...
public class Circle extends Shape implements Drawable{ //원의 면적을 구해서 리턴시켜줄수 있는 메소드 완성시켜라
	// extends 는 상속의 의미가 강하고, implements는 구현의 의미가 강하다.
	Point p;		// 두개의 클래스를 상속 받을수 없다. 가장 중요한 클래스를 상속 받고 나머지는 has a 관계로 해야한다.
	double r;
	@Override
	double area() {
		return r*r*Math.PI;
